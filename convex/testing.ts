...
export const removeSmokeTestUser = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!email.endsWith("@example.test")) throw new Error("Only smoke-test users can be removed");
    ...
    const [profiles, sessions, transactions, budgets, goals, gigProfiles, gigSources, gigEntries, gigCommitments, gigPockets, gigRules, gigSplits] = await Promise.all([
    ...
    for (const document of [...profiles, ...sessions, ...transactions, ...budgets, ...goals, ...gigProfiles, ...gigSources, ...gigEntries, ...gigCommitments, ...gigPockets, ...gigRules, ...gigSplits]) {
