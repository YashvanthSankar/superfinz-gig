# SuperFinz native app setup

The Next.js app is the web experience and API backend at the repository root. The native Expo app lives in `apps/mobile`, while browser- and device-safe gig-worker contracts and calculations live in `packages/shared`.

## Local setup

1. Install Node.js 22.13 or newer, then run `npm install` from the repository root.
2. Copy `.env.example` to `.env.local` and configure Convex, NextAuth, and Google OAuth. Set `MOBILE_JWT_SECRET` to a separate long random value.
3. Link the existing project and push the database schema/functions with `npx convex dev --configure existing --once`.
...
5. Copy `apps/mobile/.env.example` to `apps/mobile/.env` and set `EXPO_PUBLIC_API_URL` to that reachable backend URL. Set `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID` for iOS. Never put a client secret in an `EXPO_PUBLIC_` variable.
...
4. Set `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID` and either download `google-services.json` and `GoogleService-Info.plist` into `apps/mobile`, or set `GOOGLE_IOS_URL_SCHEME` to the iOS reversed client ID. The credential files are gitignored.
...
Deploy the repository root to Vercel without changing the root directory. Add all server values from `.env.example` and run `npx convex deploy --cmd 'npm run build'` as the build command. Set `NEXTAUTH_URL` to the final HTTPS URL and add its `/api/auth/callback/google` URI to the Web OAuth client.
...
The web app continues to use its NextAuth HTTP-only cookie. Native clients exchange a verified Google ID token for a 15-minute bearer access token and a rotating 30-day opaque refresh token. Only a SHA-256 hash of each refresh token is stored in Convex; device tokens are stored with Expo SecureStore. Logout revokes the server session. Protected handlers authorize bearer tokens at the route before Convex data access, while the proxy continues to enforce web-cookie access.
