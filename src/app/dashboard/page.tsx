...
export default async function DashboardPage() {
  const session = await getSession(); if (!session) redirect("/login"); if (!session.onboarded) redirect("/onboarding"); const bundle = await getGigBundle(session.userId); if (!bundle) redirect("/onboarding");
